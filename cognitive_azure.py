...
def get_caption(description) -> str:
    ...
    return f"I am {captions[0].confidence * 100:.2f}% certain that this is {captions[0].text}"\
        if captions else "No caption detected"


def get_description(image_path: str, ai_image_results):
    ...
    ai_image_results.description = get_caption(description)
